...
team_t team = {
    ...
    "Ralph Paul",     /* First member full name */
    "ralphpaul@gwu.edu",  /* First member email address */
...
/* below are the main computations for your implementation of the rotate. Any changes in implementation will go here or the other functions it may call */
	...
	for (j = 0; j < dim; j+=16)
		for (i = 0; i < dim; i+=16)
			for (blockCol = j; blockCol < j + 16; blockCol++)
				for (blockRow = i; blockRow < i + 16; blockRow++)
					dst[(e-blockCol)*dim+blockRow] = src[(blockRow * dim)+blockCol];
...
/* below are the main computations for your implementation of the smooth function. Any changes in implementation will go here or the other functiosn it calls */
	...
	for (i = 0; i < dim; i+=16)
		for (j = 0; j < dim; j+=16)
			for (blockRow = i; blockRow < i + 16; blockRow++)
				for (blockCol = j; blockCol < j + 16; blockCol++)
					dst[(blockRow * dim) + blockCol] = avg(dim, blockRow, blockCol, src);
...
void comp(int dim, int i, int j, pixel *src, pixel *dst )
{
